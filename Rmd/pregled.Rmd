...
#rade pie chart!
dff2 <- data.frame(group = c("Odrasli s djecom", "Samo odrasli"), 
...
ggplot(dff2, aes(x="", y=value, fill=group)) +
  ...
  geom_text(aes(label = paste0(round(value/nrow(hotel),2), "%")), position = position_stack(vjust=0.5)) +
  labs(title = "Analiza gostiju - dolasci s djecom/bez djece", x=NULL, y=NULL) +
...
dff <- data.frame(group = c("Zahtijeva parking", "Nije potreban"), 
